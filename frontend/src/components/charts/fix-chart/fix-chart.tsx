...
export const FixChart = ({ shiftId, list, shiftStatus }: IChartProps) => {
  ...
  const getMaxCount = () => {
    if (fixs.length === 0) return 0; // или null, в зависимости от логики
    return Math.max(...fixs.map((item) => item.count));
  };
          ...
          {!shiftId ? (
            ...
            <>
              <div className={styles.wrapper__header}>
                <span className={styles.location}>РАСКРЕПЛЕНИЕ</span>
