import type { IList, IShipment, ISuccess } from '../api.interface';
...
export const getShipmentsApi = async (
  ...
  try {
    ...
    const response = await fetch(`${URL}/shifts/${shiftId}/shipments`, {
      method: 'GET',
      headers: {
...
export const updateShipmentApi = async (
  ...
  try {
    // Здесь происходит запрос к серверу
    const response = await fetch(`${URL}/shipments/update-shipment`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json;charset=utf-8',
      },
