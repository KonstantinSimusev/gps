import type { IList, IPack, ISuccess } from '../api.interface';

// Используем переменную окружения
export const URL = import.meta.env.VITE_API_URL ?? '/api/gps';

export const getPacksApi = async (shiftId: string): Promise<IList<IPack>> => {
  try {
    // Здесь происходит запрос к серверу
    const response = await fetch(`${URL}/shifts/${shiftId}/packs`, {
      method: 'GET',
      headers: {
